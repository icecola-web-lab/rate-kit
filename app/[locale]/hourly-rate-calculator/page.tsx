...
export async function generateMetadata({ params }: LocalePageProps) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.hourlyRate.meta' });
  return buildPageMetadata({ locale, path: '/hourly-rate-calculator', title: t('title'), description: t('description') });
}

export default async function HourlyRateCalculatorPage({ params }: LocalePageProps) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations('calculators.hourlyRate');
  ...
  const schemas = [
    buildSoftwareApplicationSchema({ locale: appLocale, path: '/hourly-rate-calculator', name: t('meta.title'), description: t('meta.description') }),
    buildBreadcrumbSchema(appLocale, [
      { name: nav('home'), path: '/' },
      { name: t('title'), path: '/hourly-rate-calculator' }
    ]),
    ...
    <main className="container py-10 sm:py-14">
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_340px]">
        ...
        <aside className="grid gap-4">
          ...
          <FormulaCard title={common('formula')} formulas={[t('formulaRevenueTarget'), t('formulaHourlyRate')]} />
          <Card>
            <CardHeader>
            ...
            <CardContent className="space-y-4">
              ...
              <Link href="/day-rate-calculator" className="text-sm font-semibold text-primary">
                {t('relatedCta')}
              </Link>
            </CardContent>
          </Card>
