...
import { RetainerCalculator } from '@/components/retainer-calculator';
...
export async function generateMetadata({ params }: LocalePageProps) {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.retainer.meta' });
  return buildPageMetadata({ locale, path: '/retainer-calculator', title: t('title'), description: t('description') });
}

export default async function RetainerCalculatorPage({ params }: LocalePageProps) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations('calculators.retainer');
  ...
  const schemas = [
    buildSoftwareApplicationSchema({ locale: appLocale, path: '/retainer-calculator', name: t('meta.title'), description: t('meta.description') }),
    buildBreadcrumbSchema(appLocale, [
      { name: nav('home'), path: '/' },
      { name: t('title'), path: '/retainer-calculator' }
    ]),
    ...
    <main className="container py-10 sm:py-14">
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_340px]">
        ...
        <aside className="grid gap-4">
          ...
          <FormulaCard title={common('formula')} formulas={[t('formulaBaseRetainer'), t('formulaRetainer')]} />
          <Card>
            <CardHeader>
            ...
            <CardContent className="space-y-4">
              ...
              <Link href="/guides/hourly-vs-project-pricing" className="text-sm font-semibold text-primary">
                {t('relatedCta')}
              </Link>
            </CardContent>
          </Card>
